...
int MultilayerPerceptron::initialize(int nl, vector<int> npl) {

	for(int i=0; i<nl; i++){
		for(size_t j=0; j<npl.size(); j++){
...
void MultilayerPerceptron::forwardPropagate() {
	...
	for(int i=1; i<nOfLayers; i++){
		...
		for(int j=1; j<layers[i].nOfNeurons; j++){
			...
			*net = *(layers[i].neurons[0].w) + *(layers[i].neurons[j-1].w) * layers[i].neurons[j-1].out;

			// out = sigm (net)

			*(layers[i].neurons[j].w) = *(net) * (1 - *(net) );
